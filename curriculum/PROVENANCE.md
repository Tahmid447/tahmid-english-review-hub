...
The curriculum JSON contains no third-party images or hotlinked assets. Any future third-party visual or audio asset must have a documented commercial-use licence, creator, source URL, and local attribution record before it is added.
...
5. Run `npm run generate:curriculum` to verify the frozen `025` still matches
   its release source, then run the structured curriculum tests and review the
