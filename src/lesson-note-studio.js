...
import { BLOCK_TYPES, PERMISSIONS, METADATA_FIELDS, autoImportLessonDate, applyLessonImport, duplicateNoteDraft, newBlock, newNote, moveItem, clone, importLessonText, dateLabel } from './lesson-note-model.js?v=20260915-practice';
...
export function mountLessonNoteStudio(root,{client,teacherId,profiles=[],studentId='',noteId='',initialTab='content',createNew=false,onCount=()=>{}}) {
 ...
 let lastNotice='',duplicateSourceTitle='',dateTouched=false,cardOverview=[],imageNotice={id:'',message:''};const importImages=createImportImageQueue();
 ...
 const showError=(error,target)=>{const message=noteError(error),status=root.querySelector('[data-save-status]')||root.querySelector('[data-studio-status]');if(status)status.textContent=message;const inline=target?.closest('[data-asset-id]')?.querySelector('[data-asset-status]');if(inline)inline.textContent=message;};
 ...
 const run=async(target,fn)=>{if(busy)return;lock(true);try{await fn();}catch(error){showError(error,target);}finally{lock(false);}};
 ...
 async function open(id,{skipGuard=false,keepImages=false}={}) {
  ...
  try{const data=await api.detail(id,{teacher:true});if(disposed||token!==request)return;detail=data;note=clone(data.note);dirty=keepImages&&importImages.items.length>0;duplicateSourceTitle='';dateTouched=false;imageNotice={id:'',message:''};if(!keepImages)importImages.clear();assetDrafts.clear();rememberRoute(id,activeTab==='activity');renderEditor();}
 ...
 function renderEditor(){
  ...
  if(assetDrafts.size)root.querySelector('[data-save-status]').textContent='Unsaved image details · 画像の説明を保存してください';
 ...
 function renderImportImages(host,queue,onChange){
  host.innerHTML=`<h3>Lesson images · レッスン画像 (${queue.items.length})</h3>${queue.items.map(item=>`<article class="ln-import-image" data-import-image="${e(item.id)}"><img src="${e(item.url)}" alt="${e(item.metadata.alt_text)}"><div><strong>${e(item.metadata.title)}</strong>${checkbox('cover','Use as cover · 表紙にする',item.cover,'image')}<details class="ln-image-details"><summary>Edit image details · 画像の説明を編集</summary><div class="ln-image-detail-fields">${inputField('title','Image title · 画像タイトル',item.metadata.title,{scope:'image',max:180})}${inputField('caption','Caption · 説明',item.metadata.caption,{scope:'image',rows:2,max:2000})}${inputField('alt_text','Alt text · 画像の説明',item.metadata.alt_text,{scope:'image',max:500})}${selectField('asset_type','Type · 種類',item.metadata.asset_type,[['infographic','Infographic'],['worksheet','Worksheet'],['reference','Reference'],['teacher_attachment','Teacher attachment']],'image')}</div></details><div class="ln-actions">${button('Remove · 外す',`data-remove-import ${item.asset?'disabled':''}`)}<span class="ln-status">${item.asset?'Uploaded · 保存済み':'Uploads on Save · 保存時にアップロード'}</span></div></div></article>`).join('')}`;
 ...
 function quickImport(){
  ...
  dialog.querySelector('[data-parse]').onclick=()=>{
   try{
    ...
    preview.innerHTML=`<h3>Lesson details · レッスンの概要</h3><div class="ln-import-meta">${Object.entries(METADATA_FIELDS).map(([key,[label,max]])=>{
     ...
     return `<div>${inputField(key,label,value,{scope:'import',type:key==='lesson_date'?'date':'text',rows:key==='summary'||key==='focus'?2:0,max})}<small>${imported.explicitFields.includes(key)?'From pasted text · 入力テキストより':key==='lesson_date'?'Suggested date · 日付候補':'Suggested from lesson · 教材から作成'}</small>${key==='lesson_date'?'<p class="ln-import-current" data-imported-date></p>':''}${existing.trim()?`<p class="ln-import-current">Current · 現在：${e(key==='lesson_date'?dateLabel(existing):existing)}</p>${checkbox(key,'Replace this field · この項目を置き換える',key==='lesson_date'&&autoImportLessonDate(note,imported,dateTouched),'replace')}`:''}</div>`;
    ...
    const paint=()=>{preview.querySelector('[data-imported-date]').textContent=`Imported · 取り込み後：${imported.metadata.lesson_date?dateLabel(imported.metadata.lesson_date):'Choose a date · 日付を選択'}`;preview.querySelector('[data-import-content]').innerHTML=contentMarkup({...newNote(),...imported.metadata,content_json:{schemaVersion:1,blocks:imported.blocks}},{mode:'support',controls:false});};paint();
    preview.querySelectorAll('[data-import-field]').forEach(control=>control.oninput=()=>{const key=control.dataset.importField;imported.metadata[key]=key==='tags'?control.value.split(/[,、]/).map(t=>t.trim()).filter(Boolean):control.value;if(key==='lesson_date')importDateEdited=true;paint();});
  ...
  dialog.querySelector('[data-apply]').onclick=()=>{if(!imported)return;try{
   ...
   const fields=[...dialog.querySelectorAll('[data-replace-field]:checked')].map(c=>c.dataset.replaceField);
   note=applyLessonImport(note,imported,fields);if(importDateEdited&&fields.includes('lesson_date'))dateTouched=true;
 ...
 function cleanRequired(assetId=''){if(dirty)throw new Error('Save the text and permission changes first. · 教材と権限の変更を先に保存してください。');if(!assetId&&assetDrafts.size)throw new Error('Save the image details first. · 画像の説明を先に保存してください。');}
 ...
 function renderMedia(){
  ...
  const upload=panel.querySelector('form');upload.onsubmit=async event=>{event.preventDefault();const btn=upload.querySelector('[type=submit]'),status=upload.querySelector('[role=status]');if(busy)return;lock(true);
   try{cleanRequired();const files=[...upload.querySelector('[type=file]').files];if(files.length>6)throw new Error('Upload up to 6 images at a time. · 1回6枚まで追加できます。');let count=0;
    for(const file of files){await api.upload(note.id,file,{alt_text:upload.elements.alt_text.value+(files.length>1?` (${count+1})`:''),caption:upload.elements.caption.value,title:importImageMetadata(file.name,note.title,count+1).title,asset_type:upload.querySelector('[data-upload-field]').value},{onStatus:m=>status.textContent=`${count+1}/${files.length} · ${m}`});count++;}
    await reloadDetail();
  ...
  panel.querySelectorAll('[data-asset-id]').forEach(el=>{
   ...
   el.querySelectorAll('[data-asset-field]').forEach(c=>c.oninput=()=>{const changes=Object.fromEntries([...el.querySelectorAll('[data-asset-field]')].map(f=>[f.dataset.assetField,f.value]));if(Object.entries(changes).some(([key,value])=>value!==(asset[key]||'')))assetDrafts.set(asset.id,changes);else assetDrafts.delete(asset.id);const message=assetDrafts.has(asset.id)?'Unsaved image details · 画像の説明を保存してください':'Saved · 保存済み';el.querySelector('[data-asset-status]').textContent=message;root.querySelector('[data-save-status]').textContent=assetDrafts.size?'Unsaved image details · 画像の説明を保存してください':'All changes saved · 保存済み';});
   el.querySelector('[data-save-asset]').onclick=event=>void run(event.target,async()=>{cleanRequired(asset.id);const changes=Object.fromEntries([...el.querySelectorAll('[data-asset-field]')].map(c=>[c.dataset.assetField,c.value]));await api.assetChange(asset.id,changes,note.version);assetDrafts.delete(asset.id);imageNotice={id:asset.id,message:'Image details saved · 画像の説明を保存しました'};await reloadDetail();});
   el.querySelector('[data-set-cover]').onclick=event=>void run(event.target,async()=>{cleanRequired();await api.order(note.id,assets.map(a=>a.id),note.cover_asset_id===asset.id?null:asset.id,note.version);imageNotice={id:asset.id,message:note.cover_asset_id===asset.id?'Cover removed · 表紙を解除しました':'Cover saved · 表紙を設定しました'};await reloadDetail();});
