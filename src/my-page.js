...
function renderNext() {
 ...
 host.innerHTML=`<div class="hub-next-heading"><h2>Continue your learning · 次の学習</h2></div>${nextActionsMarkup(actions)}<nav class="hub-learning-links" aria-label="Learning shortcuts · 学習への移動"><a href="/my-page/notes">Lesson Notes · ノート一覧 →</a><a href="#favorites">Review saved phrases · 保存した表現を復習 →</a></nav>${!actions.length?`<p class="hub-next-empty">${!sourcesLoaded||overviewLoading?'Loading updates… · 更新を読み込み中…':overviewError||sourcesError?'Some updates could not load. · 一部の更新を読み込めませんでした。':'Your learning space is up to date. · 新しい更新はありません。'}</p>`:''}${overviewError?'<p class="ln-status" role="status">Lesson updates could not load. · ノートの更新を取得できませんでした。 <button type="button" data-retry-overview>Retry · 再試行</button></p>':''}`;
