...
export function practiceMarkup(b,{mode='support',controls=true}={}) {
 ...
 return `<article class="ln-block ln-tone-teal ln-practice" id="block-${e(b.id)}" data-block-id="${e(b.id)}" data-practice-block="${e(b.id)}"><div class="ln-block-label">◎ ${e(label)}</div><h3>${e(b.englishText||'Add a practice prompt · 設問を入力')}</h3>${b.difficulty?`<small class="ln-micro">${e({easy:'Easy · 基礎',medium:'Medium · 標準',challenging:'Challenging · 応用'}[b.difficulty]||'')}</small>`:''}${b.tags?.length?`<div class="ln-tags">${b.tags.map(tag=>`<span>${e(tag)}</span>`).join('')}</div>`:''}${mode==='support'&&b.japaneseSupport&&b.japaneseSupportMode!=='none'?`<details class="ln-jp-support" data-reading-detail><summary>日本語サポート</summary><p class="ln-japanese">${e(b.japaneseSupport)}</p></details>`:''}${b.hint?`<details class="ln-hint"><summary>Hint · ヒント</summary><p>${e(b.hint)}</p></details>`:''}${controls&&b.pronunciation?.enabled?`<div class="ln-actions">${button('▶ US · Ava','data-note-voice="us"')}${button('▶ UK · Libby','data-note-voice="gb"')}</div><p class="ln-status" data-voice-status role="status"></p>`:''}<div data-practice-controls><p class="ln-status">${e(b.choices?.map((v,i)=>`${String.fromCharCode(65+i)}. ${v}`).join(' / ')||'Interactive practice · 回答できる練習問題')}</p></div></article>`;
}
...
export function mountPractice(root,{block:b,attempt,api,noteId,preview=false,onChange=()=>{}}) {
 ...
 function render(){
  ...
  controls.innerHTML=`${preview?'<p class="ln-status">Preview practice · プレビューの回答は保存されません。</p>':''}<div class="ln-practice-state"><strong>${a.completed?'Completed · 完了':'Not completed · 未完了'}</strong>${a.answered_at?`<span>Last practiced · 最終回答 ${e(new Date(a.answered_at).toLocaleString())}</span>`:''}</div>${input}<div class="ln-actions">${self?button('I understand · 理解できた','data-self="understood"')+button('More review · もう一度復習','data-self="review"'):button('Check & save answer · 回答を確認・保存','data-practice-action="answer"')}${button('Reveal model · 回答例を見る','data-practice-action="reveal"')}${a.answered_at||a.answer_revealed?button(a.completed?'✓ Completed · 完了':'Mark complete · 完了にする',`data-practice-action="complete" ${a.completed?'disabled':''}`)+button('Try again · やり直す','data-practice-action="retry"'):''}</div><p class="ln-practice-outcome" role="status">${outcome}</p>${a.answer_revealed?`<div class="ln-practice-model"><strong>Model & explanation · 回答例と解説</strong><p>${e(key||'Check your expression with your teacher. · 先生と表現を確認しましょう。')}</p><p>${e(b.explanation||'')}</p>${button('Hide model · 回答例を閉じる','data-hide-model')}</div>`:''}`;
