...
assert.match(pricingUi, /import "\.\/pwa\.js\?v=20260908-campaign1"/);
...
assert.match(pricingPage, /<b>31<\/b>/);
assert.match(pricingUi, /All 31 lesson reviews/);
assert.doesNotMatch(`${pricingPage}\n${pricingUi}`, /All 17 lesson reviews|全17レッスン復習/);
...
assert.match(pricingUi, /localizedHeading\("Premium\+ Coaching", "Premium\+ コーチング"\)/);
assert.match(pricingUi, /plan-feature-details/);
assert.match(pricingUi, /What's included/);
assert.match(pricingUi, /renderPlanQuickNav/);
...
assert.match(styles, /body\[data-page="pricing"\] \.plan-grid \{[\s\S]*grid-template-columns: repeat\(2,minmax\(0,1fr\)\)/);
...
assert.match(pricingLayoutPage, /layout-preview-grid layout-two/);
assert.match(pricingLayoutPage, /data-ui-lines/);
assert.match(pricingLayoutUi, /layout-\$\{layout\}/);
assert.match(pricingLayoutUi, /\? "four" : "two"/);
...
assert.match(lessonPage, /id="openPracticeSettings"/);
assert.match(lessonPage, /id="practiceSettingsSummary"/);
assert.match(lessonPage, /settings-global-only/);
...
assert.match(lessonUi, /openSettings\("practice"\)/);
