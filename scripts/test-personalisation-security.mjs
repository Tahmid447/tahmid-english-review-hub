...
update auth.users set created_at=now() where id='${ids.studentA}';
update review_memberships set status='pending' where user_id='${ids.studentA}';`);
await as("teacherB");
assert.equal(await scalar("select review_is_site_owner()"),false);
assert.equal(await scalar("select count(*)::int from review_site_experience"),0);
...
await as("studentA");
assert.equal(await scalar("select review_my_hub_settings()->>'show_words'"),'false');
assert.equal(await scalar("select review_student_hub_feature_enabled('words')"),false);
assert.equal(await scalar("select count(*)::int from review_curriculum_items where category='words'"),0);
assert.equal(await scalar("select (review_my_experience()->>'welcome')::boolean"),true);
await db.exec("select review_dismiss_offer()");
assert.equal(await scalar("select (review_my_experience()->>'welcome')::boolean"),false);
assert.equal(await scalar("select review_my_experience()->'campaign'->'prices'->'standard'->>'offer'"),'3980');
await as("teacherA");
await db.exec(`update review_student_hub_settings set inherit_features=false,show_words=true where student_id='${ids.studentA}'`);
await as("studentA");
assert.equal(await scalar("select review_student_hub_feature_enabled('words')"),true);
await rejects(`insert into review_campaign_targets values('${ids.studentA}')`,/row-level security/);
await as("teacherA");
await db.exec(`update review_site_experience set audience='selected'; insert into review_campaign_targets values('${ids.studentA}')`);
await as(null);
assert.equal(await scalar("select review_my_experience()->'campaign'"),null);
await rejects("select * from review_site_experience",/permission denied/);
await as("studentB");
assert.equal(await scalar("select review_my_experience()->'campaign'"),null);
await as("studentA");
assert.equal(await scalar("select review_my_experience()->'campaign'->>'active'"),'true');
await as("teacherA");
await db.exec("update review_site_experience set starts_at=now()-interval '2 days',ends_at=now()-interval '1 day'");
await as("studentA");
assert.equal(await scalar("select review_my_experience()->'campaign'->>'active'"),'false');
assert.equal(await scalar("select review_my_experience()->>'welcome'"),'false');
await as("teacherA");
const notice=await scalar(`select (review_publish_announcement('{"title_en":"Private note","body_en":"Practice this week"}', '${ids.studentA}')).id`);
assert.equal(await scalar(`select count(*)::int from review_announcements where id='${notice}'`),1);
await as("studentA");
assert.equal(await scalar(`select count(*)::int from review_announcements where id='${notice}'`),1);
await as("studentB");
assert.equal(await scalar(`select count(*)::int from review_announcements where id='${notice}'`),0);
await as("teacherB");
await rejects(`select review_publish_announcement('{"title_en":"No","body_en":"No"}', '${ids.studentA}')`,/not assigned/);
await as("teacherA");
const directNotice=await scalar(`insert into review_announcements(teacher_id,title_en,body_en,audience) values('${ids.teacherA}','Returning works','Practice','all') returning id`);
assert.ok(directNotice);
await rejects(`update review_site_experience set prices='{"standard":{"offer":3980}}'`,/whole-yen/);
await as("studentA");
await rejects("select review_publish_announcement('{}',null)",/Teacher access/);
console.log("Campaign targeting, true expiry, welcome dismissal, owner-only controls, effective feature RLS and atomic announcements passed.");
await db.close();
