...
try {
  globalThis.window = { supabase: { createClient: () => ({
    ...
    rpc(name) { settingsQueries.push(name); return Promise.resolve(accessFixture.settingsResult); },
    from(table) {
...
assert.match(homePage, /src="\/src\/hub\.js\?v=20260908-campaign1"/);
assert.match(lessonPage, /src="\/src\/lesson\.js\?v=20260908-campaign1"/);
...
assert.match(hubScript, /from "\.\/supabase\.js\?v=20260908-campaign1"/);
assert.match(lessonScript, /from "\.\/effects\.js\?v=20260908-campaign1"/);
...
assert.match(serviceWorker, /CACHE_NAME = "te-review-public-v24"/);
