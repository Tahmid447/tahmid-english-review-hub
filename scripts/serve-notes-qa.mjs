...
async function api(payload){
 ...
 if(payload.op==='rpc'){
  if(!/^review_note_[a-z_]+$/.test(payload.name)||Object.keys(payload.args).some(k=>!/^[a-z_]+$/.test(k)))throw new Error('Invalid fixture RPC');
 ...
 if(payload.op==='query'){
  ...
  if(payload.single&&data.length!==1)throw new Error('Note unavailable');return payload.single?data[0]:data;
 }
...
http.createServer(async(req,res)=>{
 ...
 const pathname=url.pathname==='/'?'/scripts/qa-lesson-notes.html':decodeURIComponent(url.pathname);
